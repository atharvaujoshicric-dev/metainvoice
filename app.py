...
# ... (Include the CSV_COLUMNS and clean_amt function from the previous message) ...
    ...
    for z_idx, zip_file in enumerate(uploaded_zips):
        with zipfile.ZipFile(zip_file) as z:
            ...
            for i, pdf_path in enumerate(file_list):
                status_text.text(f"Processing {pdf_path} from Zip {z_idx+1}...")
                with z.open(pdf_path) as f:
                    # Read and process immediately to save RAM
                ...
                # Force clear memory every 10 files
                if i % 10 == 0:
                    gc.collect()

    if all_rows:
        df = pd.DataFrame(all_rows)
        # ... (rest of the Excel generation code from the previous response) ...
