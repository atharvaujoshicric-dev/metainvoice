...
if uploaded_zips:
    if st.button("▶ Process Invoices"):
        ...
        for z_idx, zip_file in enumerate(uploaded_zips):
            try:
                with zipfile.ZipFile(zip_file) as z:
                    pdf_files = [f for f in z.infolist() if f.filename.lower().endswith('.pdf')]
                    total = len(pdf_files)
                    ...
                    for i, file_info in enumerate(pdf_files):
                        status.text(f"[ZIP {z_idx+1}] {file_info.filename} ({i+1}/{total})")
        ...
        if all_data:
            ...
            output.seek(0)  # ← This was the missing fix
            ...
            st.error("No data extracted. Check your PDF format or ZIP contents.")
